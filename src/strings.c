...
void strings(char *dirtable, char current_dir_index, char *target);
            ...
            if (returncode_src == 0 && target_entry_byte != EMPTY_FILES_ENTRY) {
                if (target_entry_byte != FOLDER_ENTRY) {
                    ...
                    while (i < FILE_SIZE_MAXIMUM && null_count < 5) {
                        ...
                        if ((CHAR_SPACE <= file_read[i] && file_read[i] <= CHAR_TILDE
                        || file_read[i] == CHAR_NULL || file_read[i] == CHAR_CARRIAGE_RETURN
                        || file_read[i] == CHAR_LINEFEED))
                            putchar(file_read[i]);
                    ...
                    print("strings: not a file\n", BIOS_WHITE);
                }
            }
            else {
                print("string: ", BIOS_WHITE);
                print(source_directory_name, BIOS_WHITE);
